...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['nombre']) && isset($_POST['estadio'])) {
    ...
    if (!empty($nombre) && !empty($estadio)) {
        if ($equipoDAO->insert($nombre, $estadio)) {
            ...
            $mensaje = "<div class='alert alert-danger' role='alert'>Error al añadir el equipo. El nombre puede que ya exista.</div>";
        }
    } else {
...
// La conexión se gestiona y cierra a través del PersistentManager, no es necesario $db->close()
?>
